...
function dados_e_plot(G, schedule_matrix, d; title = "", alpha = 0.4, CVaR = zeros(12), legend = false, inverse = false, y_max = 350)
    ...
    if length(size(d)) == 1
        return schedule, plot1, groupedbar([schedule.capacity_available d], title = title, label = ["Capacidade", "Demanda"],
            legend = legend, xaxis = collect(1:nmonths), xlabel = "Meses", ylabel = "Energia", ylims = (0, y_max))
    else
        if sum(CVaR) == 0
            plot2 = groupedbar([schedule.capacity_available [maximum(d[month, :]) for month in 1:nmonths] [sum(d[month, :]) / Ns for month in 1:nmonths]],
                ...
                legend = legend, xaxis = collect(1:nmonths), xlabel = "Mês", ylabel = "Energia", ylims = (0, y_max))
        else
